...
const getCategoryIcon = (category: string) => {
  switch (category) {
    case "restaurant":
      return <UtensilsCrossed size={24} className="text-red-500" />;
    case "cafe":
      return <CoffeeIcon size={24} className="text-amber-500" />;
    case "viewpoint":
      return <Camera size={24} className="text-emerald-500" />;
    case "attraction":
      return <Landmark size={24} className="text-purple-500" />;
    default:
      return <MapPin size={24} className="text-gray-500" />;
  }
};
    ...
    const updateImages = async () => {
      const updatedSections = await Promise.all(
        sections.map(async (section) => {
          if (section.type !== "day") return section;

          const updatedPlaces = await Promise.all(
            section.places.map(async (place) => {
              if (place.locationImageUrl) return place;

              try {
                const img = await fetchLocationImage(place.name);
                return {
                  ...place,
                  locationImageUrl: img,
                };
              } catch {
                return place;
              }
          ...
          return { ...section, places: updatedPlaces };
        }),
      );
    ...
    setSections((prev) =>
      prev.map((s) => {
        if (s.id === sectionId && s.type === "day") {
          return {
            ...s,
            places: [...s.places, { ...place, order: s.places.length + 1 }],
          };
        }
        return s;
      }),
        ...
        <div className="flex flex-col gap-5 px-8 py-6 pb-5">
          {sections.map((section, idx) => (
            ...
            >
              {/* Section header */}
              <div
                className={`flex items-center justify-between px-5 py-4 transition-all ${section.type === "day" ? "bg-gradient-to-r from-indigo-50/80 to-white" : "bg-gradient-to-r from-amber-50/80 to-white"}`}
              >
                      ...
                      {section.listItems.length > 0 && (
                        <div className="space-y-2">
                          {section.listItems.map((item) => (
                            <div
                              key={item.id}
                              className="group/item flex items-center gap-3 rounded-lg p-2 hover:bg-gray-50"
                            >
                              {item.type === "checklist" ? (
                                <Checkbox
          ...
          >
            {isAddingDay ? (
              ...
              <>
                <div className="rounded-full bg-gray-100 p-1 group-hover:bg-indigo-100">
                  <Plus size={16} />
                </div>
          ...
          >
            ...
            {allMarkers.map((marker, idx) => {
              ...
              return (
                ...
                >
                  <Popup>
                    <div className="w-[280px] max-w-[280px] overflow-hidden rounded-2xl bg-transparent shadow-2xl">
                      {/* 只有非 route 类型才显示图片区域 */}
                      {marker.type !== "route" && (
                        <>
